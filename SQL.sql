...
Query: SELECT f.titulo, f.genero, s.horario, sl.nome AS sala
FROM Filme f
...
Query: SELECT filme, sala, lugares_disponiveis
...
Query: SELECT c.nome, f.titulo, s.horario, b.quantidade
FROM Cliente c
...
Query: SELECT f.titulo, SUM(b.quantidade) AS total_vendido
FROM Filme f
...
Query: SELECT id_sala, horario, COUNT(*)
FROM Sessao
